...
User Function TestGcGerarPortalExtratos()
    ...
    If nCount <> 3
        ConOut("[ERROR] [FAIL] Expected 3 records, got " + cValToChar(nCount))
        Return .F.
    EndIf
    ...
    If Len(aResult) > 0
        ...
        EndIf
    Else
        ConOut("[ERROR] [FAIL] Unit 101 record not found")
        Return .F.
    EndIf
    ...
    If Len(aResult) > 0
        ...
        Else
            ConOut("[ERROR] [FAIL] Unit 103 status incorrect: " + aResult[1]:REX_STATUS)
            Return .F.
        EndIf
    Else
        ConOut("[ERROR] [FAIL] Unit 103 record not found")
        Return .F.
    EndIf
...
User Function TestGcGerarPortalAgenda()
    ...
    ConOut("Test: GcGerarPortalAgenda(" + cCompet + ")")
    ...
    If nCount < 9
        ConOut("[ERROR] [FAIL] Expected at least 9 records (3 months x 3 units), got " + cValToChar(nCount))
        Return .F.
    EndIf
    ...
    If Len(aResult) > 0
        ...
        EndIf
    Else
        ConOut("[ERROR] [FAIL] Unit 101 record not found for 2025-01")
        Return .F.
    EndIf
    ...
    ConOut("[PASS] TestGcGerarPortalAgenda completed successfully")
Return .T.
...
User Function TestPeriodClosureGeneratesSnapshots()
    ...
    ConOut("Test: Period closure with Portal v2 snapshots for " + cCompet)
    ...
    If Len(aResult) > 0
        ...
        Else
            ConOut("[ERROR] [FAIL] Expected at least 2 extract records, got " + cValToChar(nExtratoCount))
            Return .F.
        EndIf
    Else
        ConOut("[ERROR] [FAIL] Unable to query RPT_PORTAL_EXTRATOS")
        Return .F.
    EndIf
    ...
    If Len(aResult) > 0
        ...
        Else
            ConOut("[ERROR] [FAIL] Expected at least 2 agenda records, got " + cValToChar(nAgendaCount))
            Return .F.
        EndIf
    Else
        ConOut("[ERROR] [FAIL] Unable to query RPT_PORTAL_AGENDA")
        Return .F.
    EndIf
    ...
    If Len(aResult) > 0
        ...
        EndIf
    Else
        ConOut("[ERROR] [FAIL] Unit 101 not found in extract snapshot")
        Return .F.
    EndIf

    ConOut("[PASS] TestPeriodClosureGeneratesSnapshots completed successfully - snapshots created after period closure")
Return .T.
...
User Function TestE2EPortalFlow()
    ...
    ConOut("E2E Test: Complete Portal v2 flow (close period → access portal)")
    ConOut("")
    ...
    cSql += "1)"
    TCSqlExec(cSql)
    ConOut("[PASS] Accounting entries created")
    ...
    If !GcCriarAviso("E2E Test Aviso 2", "This is the second E2E test notice")
        ...
        Return .F.
    EndIf
    ConOut("[PASS] Test avisos created successfully")
    ...
    If !GcFecharPeriodo(cCompet)
        ...
        Return .F.
    EndIf
    ConOut("[PASS] Period closed successfully")
    ...
    ConOut("Step 10: Verify snapshots were created")
    ...
    If Len(aResult) > 0
        ...
        Else
            ConOut("[ERROR] [FAIL] Expected at least 1 extract record, got " + cValToChar(nExtratoCount))
            Return .F.
        EndIf
    Else
        ConOut("[ERROR] [FAIL] Unable to query RPT_PORTAL_EXTRATOS")
    ...
    ConOut("Step 11: Verify agenda was generated for next 12 months")
    ...
    If Len(aResult) > 0
        ...
        Else
            ConOut("[ERROR] [FAIL] Expected at least 1 agenda record, got " + cValToChar(nAgendaCount))
            Return .F.
        EndIf
    Else
        ConOut("[ERROR] [FAIL] Unable to query RPT_PORTAL_AGENDA")
    ...
    ConOut("Step 12: Verify avisos are available")
    ...
    If Len(aResult) > 0
        ...
        Else
            ConOut("[ERROR] [FAIL] Expected at least 2 avisos, got " + cValToChar(nAvisoCount))
            Return .F.
        EndIf
    Else
        ConOut("[ERROR] [FAIL] Unable to query AVISOS")
        Return .F.
    EndIf
    ...
    ConOut("")
    ConOut("[PASS] TestE2EPortalFlow completed successfully - full flow from period close to portal access verified")
...
User Function TestGcCriarAviso()
    ...
    Else
        ConOut("[ERROR] [FAIL] Erro ao criar aviso")
        Return .F.
    EndIf
    ...
    If Len(aResult) > 0
        ...
        EndIf
    Else
        ConOut("[ERROR] [FAIL] Aviso não encontrado")
        Return .F.
    EndIf
    ...
    Else
        ConOut("[ERROR] [FAIL] Deveria ter rejeitado título vazio")
        Return .F.
    EndIf
    ...
    Else
        ConOut("[ERROR] [FAIL] Deveria ter rejeitado corpo vazio")
        Return .F.
    EndIf
    ...
    ConOut("[PASS] TestGcCriarAviso completed successfully")
Return .T.
...
User Function TestGcArquivarAviso()
    ...
    aResult := TCSqlQuery(cSql)
    If Len(aResult) = 0
        ConOut("[ERROR] [FAIL] Aviso de teste não criado")
        Return .F.
    EndIf
    nAvisoId := aResult[1]:AVI_ID
    ConOut("Test aviso ID: " + cValToChar(nAvisoId))
    ...
    Else
        ConOut("[ERROR] [FAIL] Função GcArquivarAviso retornou .F.")
        Return .F.
    EndIf
    ...
    Else
        ConOut("[ERROR] [FAIL] Deveria ter rejeitado ID negativo")
        Return .F.
    EndIf
    ...
    Else
        ConOut("[ERROR] [FAIL] Deveria ter rejeitado ID inexistente")
        Return .F.
    EndIf
    ...
    If Len(aResult) > 0
        ...
        Else
            ConOut("[ERROR] [FAIL] Erro ao arquivar segundo aviso")
            Return .F.
        EndIf
    EndIf
    ...
    ConOut("[PASS] TestGcArquivarAviso completed successfully")
Return .T.
...
User Function TestGcPortalCondominoV2()
    ...
    TCSqlExec(cSql)
    cSql := "DELETE FROM AVISOS WHERE AVI_ATIVO = 1 AND D_E_L_E_T_ = ' '"
    TCSqlExec(cSql)
